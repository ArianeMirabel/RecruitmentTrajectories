...
#  sub[,Seltraits]<-complete(mice(sub[,Seltraits],printFlag=F))
  #if(any(apply(sub,2,anyNA))){print(unique(sub[,"Genus"]))}
#  return(sub)})
traitsPartial_gen<-lapply(traitsPartial_gen,function(sub){
...
#  sub[,Seltraits]<-complete(mice(sub[,Seltraits],printFlag=F),4)
  #if(any(apply(sub,2,anyNA))){print(unique(sub[,"Genus"]))}
#  return(sub)})
...
## No necessary predictor selection: less than 15 variables
