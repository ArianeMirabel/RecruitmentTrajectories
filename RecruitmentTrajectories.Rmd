...
source("Scripts/GithubPages.R");source("Scripts/DrawingTools.R")
```
...
The main determinant identified and retained over decades of studies on forest dynamics rely on the demographic processes determining trees recruitment. 
...
load("DB/RecruitmentAccum")
load("DB/RecruitmentPunctual")

par(mfrow=c(2,3),oma=c(1,1,1,1),no.readonly=TRUE)
for(q in 1:3){
TrajectoryRec(RecAccum)
TrajectoryRec(RecPun)
mtext("(a) Richness",at=0.13,line=-1,outer=TRUE);mtext("(b) Shannon",at=0.47,line=-1,outer=TRUE)
mtext("(c) Simpson",at=0.79,line=-1,outer=TRUE)
mtext("Years since disturbance",side=1,at=0.85,cex=0.8,line=-2,outer=TRUE)
...
The null model randomly sampled the adequate number of recruited species, as observed in inventories, in the real 5-years laps inventory. 

```{r Fig2, out.width = '100%', fig.asp = 1.2/2, echo=FALSE,fig.cap="Punctual **(a)** and accumulated **(b)** recruitment diversity trajectories for the Richness, Shannon and Simpson diversity compared to the null model trajectories. Values reported correspond to 0.5 quantile of plot-level diversity obtained after 50 repetition of the taxonomic uncertainty propagation process, averaged among plots of the same treatments. Lines colors correspond to the logging treatment initially applied (green for control, blue for T1,orange for T2 and red for T3)."}
```

```{r Fig3, out.width = '100%', fig.asp = 1.2/2, echo=FALSE,fig.cap="**(a)** Community weighted means (CWM) of the four disturbance treatment for the six leaf and stem functional traits considered and **(b)** Functional richness of the punctual recruited communities measured as the convex hull volume defined by these same functional traits. Values reported correspond to 0.5 quantile of plot-level CWM and functional richness obtained after 50 repetition of the taxonomic uncertainty propagation and the functional database gap-filling processes. Lines colors correspond to the logging treatment initially applied (green for control, blue for T1,orange for T2 and red for T3)."}
```
...
```{r Fig4, out.width = '100%', fig.asp = 1.2/2, echo=FALSE,fig.cap="Trajectories over the 30 sampled years of the abundance turnover value between recruited trees and intial stands before exploitation at plotscale. 50 repetitions of the taxonomic propagation process and tree resampling returned the quantiles of the distribution of turnover values. Lines correspond to the within treatement average of the 0.5 plot-based percentile (green for control, blue for T1,orange for T2 and red for T3). The grey envelope corresponds to the 0.025 and 0.975 distribution percentiles."}
```
